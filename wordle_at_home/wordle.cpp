...
Wordle::Wordle(Constants &c, Player &p, size_t wordLength)
    : constants(c), player(p) {
  std::random_device rd;
  randomState = std::mt19937_64(rd());
  this->wordLength = wordLength;
  MAX_GUESSES = 6 + player.upgrades.count(ADD_ATTEMPT) +
                player.upgrades.count(REVEAL_YELLOW);
  ...
  if (currentUserInput.size() == currentWord.size()) {
    ...
    if (done) {
      ...
      player.money += (10 + 2 * player.upgrades.count(MONEY_END)) *
                      (1 + player.upgrades.count(MONEY_MULTIPLIER));
      if (player.upgrades.count(SECOND_WORD_MULT) &&
          userInputHistory.size() == 2) {
        player.money *= 2;
      }
...
std::wstring Wordle::getRandomWord(int length) {
  ...
  if (player.upgrades.count(X_WORD)) {
    std::wstring word;
    while (!std::any_of(word.begin(), word.end(),
                        [](wchar_t c) { return std::towupper(c) == L'X'; })) {
...
void Wordle::receiveInput(const sf::Event &ev) {
  if (ev.is<sf::Event::TextEntered>()) {
    ...
    if (tEv->unicode > 31) {
      ...
      currentUserInput.resize(std::max<size_t>(1ul, currentUserInput.size()) -
                              1);
    } else {
      return;
    }
...
void Wordle::render(sf::RenderTarget *target) {
  ...
  for (int i = 0; i < userInputHistory.size(); ++i) {
    ...
    for (int j = 0; j < w.size(); ++j) {
      sf::FloatRect tileBounds =
          layout.getElementBounds(i * currentWord.size() + j, 2);
      ...
      case EXISTS:
        ...
        break;
      case DOES_NOT_EXIST:
        fillColor = sf::Color(128, 128, 128, 128);
        break;
      case AT_RIGHT_POSITION:
  ...
  for (int j = 0; j < w.size(); ++j) {
    sf::FloatRect tileBounds = layout.getElementBounds(
        userInputHistory.size() * currentWord.size() + j, 2);
...
bool Wordle::getPlayerWon() { return playerWon; }
...
void Wordle::makeCharLayoutRow() {
  ...
  for (int i = 0; i < currentWord.size(); ++i) {
    layout.addElement(Layout::Element{
        2,
        sf::Vector2f(WIDTH_PER_CHAR * i + CHAR_TILE_PADDING + loffset,
                     WIDTH_PER_CHAR * userInputHistory.size() +
                         CHAR_TILE_PADDING),
