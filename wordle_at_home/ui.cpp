...
void UI::renderUpgradeBar(sf::RenderTarget *target, Constants &constants,
                          Player &player, Layout::Layout &layout) {
  ...
  sf::FloatRect barRect(
      sf::Vector2f(bounds.position.x + 8.0f,
                   bounds.position.y + bounds.size.y - barHeight),
      sf::Vector2f(bounds.size.x - 16.0f, barHeight - 8.0f));
  ...
  float padding = 6.0f;
  float slotW =
      (barRect.size.x - (upgradeCount + 1) * padding) / (float)upgradeCount;
