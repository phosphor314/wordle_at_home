...
/*struct EaseOutElastic {
    float operator()(float x) const{
        const float c4 = (2 * std::numbers::pi / 3);
                    ...
                    return std::pow(2, -10 * x) * std::sin((x * 10 - 0.75) * c4)
+ 1;
    }
};*/
...
struct EasinInOutExpo {
  float operator()(float x) const {
    return x < 0.5 ? std::pow(2, 20 * x - 10) / 2
                   : (2 - std::pow(2, -20 * x + 10)) / 2;
  }
};

struct EaseOutExpo {
  float operator()(float x) const { return 1 - std::pow(2, -10 * x); }
};
} // namespace easingFuns

template <Vector T, typename easingFun = easingFuns::Linear>
  requires std::invocable<easingFun, float> &&
           std::same_as<std::invoke_result_t<easingFun, float>, float>
struct Interpolated {
public:
  ...
  explicit Interpolated(T val, float transition_speed_s = 1.0f,
                        bool clamp = true) {
    this->begin = val;
    this->end = val;
