...
sf::FloatRect Layout::Layout::getContainerBounds(size_t containerInd) const {
  ...
  float shortDim;
  if (isnan(c.divisionParam)) {
    shortDim = (1.0f - divParamSum) / nanCount;
  } else {
    shortDim = c.divisionParam;
  }
  sf::FloatRect relBounds;

  switch (parent.divisionType) {
  case LATERAL:
    relBounds = sf::FloatRect(sf::Vector2f(nearEdge, 0.0f),
                              sf::Vector2f(shortDim, 1.0f));
    break;
  case VERTICAL:
    relBounds = sf::FloatRect(sf::Vector2f(0.0f, nearEdge),
                              sf::Vector2f(1.0f, shortDim));
    break;
  case NONE:
  ...
  relBounds.position = relBounds.position.componentWiseMul(parentBounds.size) +
...
sf::FloatRect Layout::Layout::getElementBounds(size_t elementInd,
                                               size_t containerInd) const {
  ...
  return sf::FloatRect(e->pos * parentBounds.size.x + parentBounds.position,
                       e->extent * parentBounds.size.x);
}

Layout::Element &Layout::Layout::getElement(size_t elementInd,
                                            size_t containerInd) {
  Element *e = nullptr;
  size_t counter = 0;
...
void Layout::Layout::subdivideContainer(size_t container, float divisionParam,
...
void Layout::Layout::addElement(Element e) { elements.push_back(e); }
